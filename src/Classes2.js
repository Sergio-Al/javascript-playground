// We create TypedMap a subclass of Map that checks key and value types
class TypedMap extends Map {
  constructor(keyType, valueType, entries) {
    ...
    // Initialize the superclass with the (typed-checked) initial entries
    super(entries);
  ...
  set(key, value) {
    ...
    if (this.value && typeof value !== this.valueType) {
      throw new TypeError(`${value} is not the type ${this.valueType}`);
    }

    // If the types are correct, we invoke the superclass's version of the
    // set() method, to actually add the entry to the map. And we
